...
Industrielle Trennung **Engineering ↔ Operations** (Plan: [doku/program_ide_plan.md](doku/program_ide_plan.md); Phasen 0–6 implementiert, Phase 7 Pilz/KRL **funktional abgeschlossen** (Vokabular, sequenzielles Pilz-Routing, Blend-/Sequence-Modus mit circ — Blend real bestätigt; echtes Live-Override als Limitation eingeordnet: braucht Stop+Replan), Phase 8 Robustheit **in Arbeit** (Loader-Unit-Tests `ament_add_gtest` 25/25 grün, README/Roadmap aktualisiert; offen: Executor-Integrationstest):

- **Engineering (VS Code)**: Programme/Punkte als YAML in `programs/` (git-versioniert). Live-Validierung über JSON Schemas in `doku/schemas/` (`.vscode/settings.json`-Mapping, Extension `redhat.vscode-yaml`, Snippets `r0192-program`/`ptp`/`lin`/`circ`/`move_j`/`move_l`/`wait`/`point-joint`/`point-pose`). `.gitignore` whitelisted `.vscode/{settings,extensions,r0192.code-snippets}`.
...
- **Datenmodell**: Punkte (`points.yaml`, Map by name, `type: joint` mit 6 rad-Werten joint_1..6 oder `type: pose` in `base_link`) ↔ Programme (`program_*.yaml`, Referenz **nur per Punktname**, `velocity`/`acceleration` (bzw. `vel`/`acc` bei KRL-Steps) = MoveIt-Skalierungsfaktoren (0, 1], optionale `defaults`, Fallback 0.1). **Zwei koexistierende Step-Vokabulare**: Legacy `move_j`/`move_l`/`wait` (OMPL / KDL `computeCartesianPath` / Sleep, unverändert) und **KRL/Pilz `ptp`/`lin`/`circ`** (`ptp`→Pilz PTP, `lin`→Pilz LIN, `circ`→Pilz CIRC). `move_j`/`move_l` sind als *deprecated, prefer ptp/lin* markiert. **Zwei Run-Modi** (Goal-Feld `blend`): „stop at each point" (sequenziell, je Step ein Plan, `c_dis` ignoriert, `circ` abgelehnt) und **„blend through"** (zusammenhängende Move-Steps als ein Pilz-`MotionSequenceRequest` an `/sequence_move_group`, `blend_radius=c_dis`, `circ` per „interim"-Constraint; Capability in `moveit.launch.py`). **Simulationsmodus** (Goal-Feld `dry_run`): plant alles als Pilz-Sequenz mit `plan_only`, animiert die Trajektorien als RViz-Geist auf `/display_planned_path`, bewegt den Arm **nicht** und ändert den Zustand **nicht** (kein MOVEIT), läuft aus jedem Zustand. Beide als UI-Toggle im ProgramPanel.
...
- **Ausführung v1**: sequenziell via `MoveGroupInterface` (lazy erstellt — move_group startet 3 s nach dem Executor). `move_j` akzeptiert joint+pose-Punkte. **`move_l` (Phase 6)**: `computeCartesianPath` (Param `cartesian_eef_step`, 5 mm), **Retiming serverseitig** (Skalierung×Override im Request; move_group hat die `joint_limits.yaml`-Beschleunigungslimits, das Client-Modell nicht — clientseitiges TOTG scheitert daher); `fraction < 99.9 %` → sauberer Abbruch; Null-Distanz = No-op. **KDL kann keine Linie durch die joint_5=0-Handgelenk-Singularität** (Home + alle ursprünglichen Demo-Punkte liegen dort!) — `move_l`-Ziele brauchen abgewinkeltes Handgelenk (Demo: `program_linear_demo.yaml` mit `bent_a`/`bent_b`/`lin_bent`). Cancel → `MoveGroupInterface::stop()` bricht blockierendes plan/execute, Ergebnis CANCELED. Feedback: Step-Index/Label/Total + Status (LOADING/PLANNING/MOVING/WAITING/PAUSED).
- **Punkt-Visualisierung (Phase 6)**: latched `MarkerArray` auf `/program_points_markers` — Kugel + Namens-Label je Punkt (pose = orange direkt, joint = blau via FK sobald MoveGroup verfügbar); Republish bei Teach/Delete/List; Display „ProgramPoints" in moveit.rviz. Keine interaktiven Marker (kein Edit in RViz by design).
...
- [x] Phase 5 Acceptance real bestätigt (Override 0.21 → ~4× langsamer, Pause/Resume über Panel, 2026-06-12)
- [x] Phase 6 `move_l` + Punkt-Visualisierung: serverseitiges Retiming, Singularitäts-Limitation dokumentiert, Marker auf `/program_points_markers`, Demo `program_linear_demo.yaml` — headless getestet
- [ ] Phase 2 Acceptance manuell in VS Code verifizieren (Snippet → Live-Validierung → Autocomplete/Hover)
- [ ] Phase 6 Acceptance manuell in RViz verifizieren (`program_linear_demo.yaml` sichtbar PTP+LIN, Punkte benannt im 3D-View)
- [ ] Hardware-Test: Demo-Programm gegen Achse 1/4 fahren
- [ ] Phase 7+: Pilz (`move_ptp`/`move_lin`/`move_circ`, Blending, Live-Override), Phase 8 Robustheit/Tests (siehe Plan)
